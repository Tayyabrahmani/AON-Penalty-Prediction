...
### Set core controls
apollo_control = list(
  modelName  ="Apollo_6_alt_modelNew",
...
database = read.csv("/Users/nitishaggarwal/Desktop/Project_1_penaltyShot (2)/Project/6altallcolumn.csv",header=TRUE)
names(database)[1] <- "ID"
View(database)
...
modelNew = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)

apollo_modelOutput(modelNew)

forecastNew = apollo_prediction(modelNew,
                               apollo_probabilities,
                               apollo_inputs)
apollo_saveOutput(modelNew)