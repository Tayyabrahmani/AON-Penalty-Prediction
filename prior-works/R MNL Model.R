...
### Set core controls
apollo_control = list(
  modelName  ="Apollo_finished_model",
...
#database = subset(database2,database2$RP==1)
...
#library(dplyr)
#database <- mutate(database,test = 4 )
#for (i in 1:2358) {
#  if (database[i,6] == 0)
#  {database[i,132] <- 1 }
#  if (database[i,6] == 1)
...
choiceAnalysis_settings <- list(
  ...
  explanators  = database[,c("foot","nmovGK")]
)
...
apollo_fixed = c("asc_DC","b_foot5", "b_moveGK5", "b_GKS5", "b_IS5","b_comL5", "b_HGK5", "b_dec5", "b_lp5", "b_latr5", "b_ladr5", "b_solsbg5","b_rnog5", "b_def5", "b_nmovGK5")
...
apollo_probabilities=function(apollo_beta, apollo_inputs, functionality="estimate"){
  ...
  V[['DL']]  = asc_DL + b_foot4*foot + b_moveGK4*moveGK  + b_GKS4*GKS + b_IS4*IngSo + b_comL4*comLea + b_HGK4*HeiGK + b_dec4*Dec + b_lp4*lpbg  + b_latr4*la3 + b_ladr4*la6                + b_perc*perc4   + b_rnog4*rnoGroup + b_solsbg4*Solsbg  + b_def4*def + b_nmovGK4*nmovGK 
  V[['DC']]  = asc_DC + b_foot5*foot + b_moveGK5*moveGK  + b_GKS5*GKS + b_IS5*IngSo + b_comL5*comLea + b_HGK5*HeiGK + b_dec5*Dec + b_lp5*lpbg  + b_latr5*la3 + b_ladr5*la6                + b_perc*perc5   + b_rnog5*rnoGroup + b_solsbg5*Solsbg  + b_def5*def + b_nmovGK5*nmovGK 
  V[['DR']]  = asc_DR + b_foot6*foot + b_moveGK6*moveGK  + b_GKS6*GKS + b_IS6*IngSo + b_comL6*comLea + b_HGK6*HeiGK + b_dec6*Dec + b_lp6*lpbg  + b_latr6*la3 + b_ladr6*la6                + b_perc*perc6   + b_rnog6*rnoGroup + b_solsbg6*Solsbg  + b_def6*def + b_nmovGK6*nmovGK 
...
modelnNM = apollo_estimate(apollo_beta, apollo_fixed, apollo_probabilities, apollo_inputs)
...
apollo_modelOutput(modelnNM)

# ----------------------------------------------------------------- #
#---- FORMATTED OUTPUT (TO FILE, using model name)               ----
# ----------------------------------------------------------------- #

apollo_saveOutput(modelnNM)

forecastnNM = apollo_prediction(modelnNM,
...
apollo_lrTest("Apollo_pencNM_151","Apollo_pencNM_150")
